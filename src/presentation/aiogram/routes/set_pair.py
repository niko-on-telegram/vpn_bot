...
from src.presentation.aiogram.keyboards import post_order_keyboard
from src.presentation.aiogram.keyboards import submit_keyboard
from src.presentation.aiogram.schema import CreateChatPairSchema
from src.presentation.aiogram.schema import retort
...
@router.message(StateFilter(SetPairSG.PUBLIC))
async def set_public_chat(message: Message, state: FSMContext) -> None:
    await state.update_data(public_chat_id=message.text)
    await state.set_state(SetPairSG.PRIVATE)

    await message.answer('Enter private chat id or username')


@router.message(StateFilter(SetPairSG.PRIVATE))
async def set_private_chat(message: Message, state: FSMContext) -> None:
    await state.update_data(private_chat_id=message.text)
    await state.set_state(SetPairSG.ORDER)
    await message.answer('Enter post send order', reply_markup=post_order_keyboard())


@router.callback_query(
    StateFilter(SetPairSG.ORDER), F.callback_data.in_(SendOrderEnum.values())
)
async def set_send_order(query: CallbackQuery, state: FSMContext) -> None:
    logger.info('Set send order')
    await state.update_data(send_order=query.text)
...
@router.callback_query(StateFilter(SetPairSG.SUBMIT), F.callback_data == 'no')
async def cancel_submit(query: CallbackQuery, state: FSMContext):
    await state.clear()
    await query.answer('Cancelled!')


@router.callback_query(StateFilter(SetPairSG.SUBMIT), F.callback_data == 'yes')
async def submit(
