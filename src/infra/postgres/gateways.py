...
from src.infra.postgres.tables import ChatPairModel


class BasePostgresGateway:
    def __init__(
        self, retort: Retort, session: AsyncSession, table: type[BaseDBModel]
    ) -> None:
        self.retort = retort
...
class ChatsGateway(BasePostgresGateway):
    def __init__(self, session: AsyncSession) -> None:
        super().__init__(retort=Retort(), session=session, table=ChatPairModel)

    async def get_all_pairs(self) -> list[ChatPairSchema]:
        return self.retort.load(
            (await self.session.execute(select(ChatPairModel))).mappings().fetchall(),
            list[ChatPairSchema],
        )

    async def create_pair(
        self, private_chat_id: str, public_chat_id: str, send_order: SendOrderEnum
    ) -> ChatPairSchema:
        stmt = (
            insert(ChatPairModel)
            .values(
                private_chat_id=private_chat_id,
                public_chat_id=public_chat_id,
                send_order=send_order,
            )
            .returning(ChatPairModel)
        )
        try:
            return self.retort.load(
                (await self.session.execute(stmt)).mappings().fetchall(), ChatPairSchema
            )
        except IntegrityError as e:
